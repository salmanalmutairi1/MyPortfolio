          ...
          <div className="col-span-12 lg:col-span-5">
            <Reveal>
              ...
              <ul className="border-y border-[var(--border-strong)]">
                {channels.map((c, idx) => {
                  const I = c.Icon as React.FC<{ size?: number }>;
                  return (
                    <li key={c.name}>
                      <a
                        href={c.href}
                        target={c.name !== 'Email' && !c.name.includes('البريد') ? '_blank' : undefined}
                        rel="noreferrer"
  ...
  return (
    <div>
      ...
      <Cmp
        ...
        className="w-full bg-transparent border-0 border-b border-[var(--border-strong)] px-0 py-3 text-[var(--text-primary)] placeholder:text-[var(--text-muted)] focus:outline-none focus:border-[var(--accent)] transition-colors resize-none font-mono text-sm tracking-wide"
      />
    </div>
  );
};
